...
- **Position: text-block center ~0.60 of frame height** (upper chest, just below the chin) — NOT lower-third. Verified 2026-05-21 by overlaying a screen-center line on the real export: caption block sits at ~0.60 (puts text + emoji near screen-center = the Submagic look). `vertical_pos` defaults to 0.60 for 9:16.
...
- **Emoji position (MOST important):** emoji **HUGS the subtitle** — centered horizontally, just **below** the text, near screen-center (reference: ~0.63 frame height, directly under the caption). It does NOT fly to the frame edges / park far above / sit off to the side. Every preset RESTS centered-just-below-text; only the **entrance direction** varies. `PRESET_PATH` amplitudes are deliberately small (≤0.13 width / ≤0.07 height) — bigger versions read as "too far out, too far from the subtitle."
- **Emoji entrance presets** rotate (all settle centered-below): `slide_across` (short slide-in from the left, NOT edge-to-edge), `fly_out` (drifts up to just under the text), `slide_up` (small rise from below), `pop` (scale in place). `EMOJI_ENTER_DUR 0.45s`, smooth ease `1-(1-p)³`.
- **Emoji cadence + linger:** one emoji at a time, on keyword cards, ~every **1.5s** (`EMOJI_MIN_GAP 1.5`); each lingers until the next replaces it, up to `EMOJI_LINGER 2.6s` for trailing gaps (no flicker / "disappears too quickly"). A 2.5s gap was too sparse.
- **Emoji art = PREFER the animated Noto GIF** when one exists for the glyph (animated beats static — what Submagic uses for lively ones), else **Apple Color Emoji static** (exact-Apple look, PIL `embedded_color` 160px sbix strike, single-codepoint only — can't shape ZWJ families/flags), else Twemoji PNG. `.nogif` marker caches 404s. All get the transform motion regardless of art source. There is **no free animated-Apple set** (Apple's font is static; "Apple-styled animated" Lottie packs are paid). Apple-emoji licensing is a gray area for commercial video.
