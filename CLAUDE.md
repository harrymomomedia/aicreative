...
#### gpt-image-2 i2i DOES preserve identity — the JSON key is `input_urls`, NOT `image_urls` (root-caused 2026-07-11)

The long-standing belief that "gpt-image-2 i2i re-imagines the face / can't hold identity" was a **payload bug, not a model limitation.** KIE's `gpt-image-2-image-to-image` model reads the reference images from **`input_urls`**. If you send them under `image_urls`, KIE **silently ignores them and falls back to text-to-image** → it invents a brand-new face/scene from the prompt alone (this is what produced every "different person" result: the 4K-upscale swap, the two-shot composites, the news-composite drift). **`kie_client.generate_gpt_image` now sends `input_urls`** (the Python kwarg stays `image_urls` for back-compat — only the JSON key changed). Verified 2026-07-11: same-face round-trip now preserves the exact face, hair, and even a small forearm tattoo. Rules for reliable i2i identity:

- **JSON key MUST be `input_urls`** (already fixed in the client). If a face comes back as a stranger, this is the first thing to check.
- **Keep the i2i prompt MINIMAL — do NOT describe the person.** Describe only the scene/action/composition ("this exact woman, standing on a sidewalk, being interviewed"). Re-describing appearance fights the reference and drifts identity. (Long describe-the-subject prompts were the second half of the old failure.)
- **Always `kie.upload_file(path)` FIRST and pass the hosted URL.** `generate_gpt_image` does NOT auto-upload — passing a **local file path** as a reference → KIE can't fetch it → job returns `state: fail`.
- `input_fidelity="high"` is the client default (preserves input features); set `"low"` only when you deliberately want a loose re-interpretation.
- **Multi-subject composites (two-shot anchors) now work faithfully** via `input_urls=[personA, personB]` + a minimal composition prompt — this is the correct path for the vox-pop interviewer+respondent two-shot and any news-composite. nano-banana is still fine but no longer *required* just to hold a face.
...
**For pure UPSCALING, use Real-ESRGAN — i2i is not an upscaler** (it re-renders at the target size, it does not add true resolution to the exact pixels). NOTE (updated 2026-07-11): the old "same man at 4K came back as a completely different person" failure was **compounded by the `input_urls` payload bug** (see the i2i identity section above) — with `input_urls` + a minimal prompt, i2i now *does* hold the face. But even with identity preserved, i2i still re-renders rather than super-resolving, so for a crisp higher-res version of an EXACT picked image, Real-ESRGAN remains the right tool.
...
**Background-preserving person swaps: nano-banana OR corrected gpt-image-2 i2i (2026-06-12; updated 2026-07-11).** If the user gives an existing photo and says "keep this exact background, only change the person," `generate_nano_banana` is a reliable strict edit model. NOTE: the old claim that "`gpt-image-2` i2i re-stages the whole scene and beautifies the face" was largely the **`input_urls` payload bug** (fixed 2026-07-11 — see the i2i identity section) — with the correct `input_urls` key + a minimal prompt, gpt-image-2 i2i now performs faithful subject-preserving edits too. Either model works now; keep prompts minimal. Preserve crop, camera angle, lighting, reflections, seat/interior geometry, and replace only the subject. After picking the right face, re-run **one locked variant** for the required 2K portrait output instead of broad batch re-generation.
