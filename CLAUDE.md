...
- **Animation:** captions stay ON continuously (no per-word flash); text pop 96%→105%→100% over 0.12s; **emoji entrance presets rotate** `slide_across` (left edge→right edge, full traverse, smooth easeOut), `fly_out` (center→up/45° outward, rests above), `slide_up`, `pop` — each rests in a different spot, near screen-center, just above/below the text. `EMOJI_ENTER_DUR 0.45s`.
- **Emoji = HYBRID** (verified by frame-diff: Submagic's parked emojis are static; the "animation" is mostly the *movement*, with a *few* genuinely animated). So: **Apple Color Emoji (static, exact look) for most** (rendered via PIL `embedded_color` at the 160px sbix strike — single-codepoint only, can't shape ZWJ families/flags), **Noto animated GIF only for the lively set** `ANIMATE_VIA_NOTO` (🔥 ⏳ ❤️ ✅ ❌ 💰 …). Both get the transform motion. There is **no free animated-Apple set** (Apple's font is static; "Apple-styled animated" Lottie packs are paid — iconscout/creattie). Apple-emoji licensing is a gray area for commercial video.
