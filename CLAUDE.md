...
- **Clinical/medical stills as i2v anchors** — Veo 3.1 **Lite** content-rejects i2v on clinical imagery (brain-scan / MRI / medical-chart stills, Depo docu b-roll 2026-07); the SAME anchor+prompt passes on Veo 3.1 **Fast**. This is the sanctioned Lite→Fast escalation case: don't burn Lite re-rolls on a clinical still, switch tiers.
...
Build EVERY static image ad (the whole banner — headline, layout, on-image text, photo, CTA, footnote) as **one `kie_client.generate_gpt_image` render**. Do **NOT** composite the headline/text programmatically (PIL, ffmpeg `drawtext`, code overlay) — the model renders the entire creative. **This OVERRIDES the older "text-precise cards render cleaner in PIL / CAWP-F1" guidance** in `inventory/depo_provera_ad_formats.md` and the `cawp-f1` work: even dense-text formats (qualify checklist, FAQ, news headline, dictionary definition, facilities map) go through gpt-image-2. To get clean text, **specify the EXACT on-image text verbatim in the prompt** and append *"Render ONLY the exact text specified — no extra words…"* The `image-ad-formats` skill is the format catalog (format × style × angle); reusable generators that follow this rule: `scripts/il_jdc_image_ads.py` (10 UGC longform + designed), `scripts/il_jdc_news_ads.py` (10 news-headline-article banners), `scripts/depo_formats100_gen.py`, `scripts/depo_meningioma20_gen.py` (meningioma-only targeting), `scripts/ccwf_jdcstyle20.py` (20 distinct designed/news formats), `scripts/ccwf_social10.py` (unbranded social-proof). Matching resumable AdMachin draft-staging scripts (one-writer state file each): `scripts/il_jdc_imageads_stage.py`, `scripts/depo_meningioma_admachin_stage.py`, `scripts/ccwf_launch30_stage.py` (this one also burns the Pulaski/Jones disclaimer BAR onto each image and appends the verbatim disclaimer to the FB primary before assembling — the CCWF-campaign pattern).

**Social-proof formats must be UNBRANDED (user-locked 2026-06-26).** Comment-thread / chat / forum-post image ads may NOT carry any real platform's branding — no Facebook logo or look-alike chrome, no Reddit, Apple/iMessage, WhatsApp, or Messenger logos. Render generic comment threads, chat bubbles, and forum posts instead (an ad that impersonates FB UI inside FB is a rejection/impersonation risk). Reference: `scripts/ccwf_social10.py`.

**gpt-image-2 child-safety gotcha on legal ads:** rendering "kid/child + sexually abused by staff" *on the image* trips KIE's `failMsg: "The input or output was flagged as sensitive."` (same child-safety pattern as Veo). Fix = soften the ON-IMAGE wording institutionally ("Held in an Illinois juvenile detention center", "youth-center sexual abuse", lawsuit/allegation framing) while keeping the fuller "sexually abused as a kid" copy in the **FB primary text** (which is not image-moderated). A first-person whistleblower "what I saw" account of youth sexual abuse is too sensitive to render — use an accountability/lawsuit angle instead. Compliance copy is unchanged ("may qualify for significant compensation", real facilities, no owed/paid/settlement, no disclaimer lingo on the creative).

**Extending a big image-ad campaign: keep a NO-REPEAT format list.** New batches must exclude every format already run in the campaign (jdcstyle20 excluded all 74 prior CCWF formats — the exclusion list lives in the script docstring) and cap testimonial-style cards at ~1 per batch. Format variety is the point of a new batch; a "new" batch that re-treads ID-card/poll/map/checklist formats is wasted spend. A "Reddit-STYLE" layout with a made-up community label (`r/Ex_Inmates`) is fine under the UNBRANDED rule above — recognizable platform branding is not.

**Disclaimer-bar campaigns: bar-burn in POST, never in the gen prompt; Depo/IL-JDC stay disclaimer-free on the creative** (`feedback_no_disclaimer_lingo_in_copy`). See `scripts/ccwf_launch30_stage.py` in the generator list above for the women's-prison burn→upload→assemble pattern.
...
- **Narration / docu VO must use `eleven_v3`** (user-locked 2026-07-09): `eleven_multilingual_v2` reads FLAT on documentary narration — the user flagged the VO as "emotion not realistic" and the cause was the MODEL, not the voice; the same voice on v3 delivers real emotion. (Depo docu narrator lock: Jessica Anne Bogart, voice_id `lxYfHSkYm1EzQzGhdbfc`, stability 0.5, similarity 0.8.)
...
### Silent b-roll i2v → omni-flash, NOT Veo (2026-07-09, Depo docu)
...
| `video-style-interview` | "interview video/ad", "matching between speakers", "two speakers sound the same", "voice consistency across clips", "interviewer + subject" | GENERAL cross-cutting interview patterns — read FIRST for any interview video. Speaker matching (distinct speakers via one-per-clip / per-speaker `voice_changer`; same-speaker consistency via `voice_consistency.py`+`audio_match.py`, sim≥0.85/ΔF0≤8Hz, STS pitch-delta zones), `input_urls` i2i identity, Grok-for-listeners routing, gaze geometry, dialogue locks, QA gates, assembly primitives (word-anchored turn `align()`, word-aware trim, stereo-concat rule, static-gain loudness). |
| `video-style-street-interview` | "street interview", "vox pop", "man on the street", "sidewalk interview", "reporter with a mic" | OUTDOOR vox-pop pipeline — street personas, two-shot i2i anchor, Grok/Veo routing, distinct-voice swap, wide-16:9 → punch-in-to-9:16 reframe, Chowchilla pron lock (Chowchilla/CCWF build). |
| `video-style-stacked-documentary-interview` | "stacked interview", "stacked podcast", "documentary interview ad", "interview with b-roll" | INDOOR seated pipeline — the 10 same-room anchor rules (reverse-angle chest-up, iPhone look, 3/4 gaze, facing-each-other flip), Grok clean talkers + silent listeners, stacked/cut assembly (stereo rule, static-gain, delogo, hybrid fallback), b-roll-over-VO, AdMachin B-Roll library (Depo build). |

**Interview-video skill family (created 2026-07):** three skills sharing the `video-style-`
prefix — one GENERAL base (`video-style-interview`, cross-cutting patterns, read first) + two
format skills (`-street-interview` outdoor vox-pop, `-stacked-documentary-interview` indoor
seated). **Structure lesson:** a general base + format-specific skills beats one giant skill (the
right one auto-surfaces on its own triggers). **Grouping lesson:** grouped by a shared NAME prefix
with **flat folders** — NOT nested under a `video-style/` subdir — because the skill loader may
only scan one level deep, and nesting risks them silently not surfacing. **Where the code lives:**
the full interview production scripts (`scripts/wp_voxpop_*`, `wp_interview_*`, `depo_interview_*`,
`depo_grok_*`, `depo_stacked_*`, `insider_*`), the `generate_grok` + `upload_broll_clip` helpers,
and the fuller **`input_urls` i2i identity fix** writeup live on branches
`claude/womens-prison-video-xvzdh8` (street/vox-pop) and `claude/ugc-video-style-q4vcfx` (stacked
depo) — NOT yet on `main`. Port those from the branches rather than re-deriving. The skills bake in
the recipes so they're self-contained even before those branches merge.
...
**Reuse proven copy rows by ID instead of re-creating them.** When a headline/primary is already approved and performing (e.g. the IL JDC headline "TAP TO SEE IF YOU QUALIFY", copy row `33205221-a2d0-4566-a132-f63452023344`), assemble new ads against the EXISTING copy row id rather than creating a duplicate row per ad — keeps the copy library deduped and insights aggregated on one row. Pattern: `scripts/il_jdc_imageads_stage.py` (`HEADLINE_ID` reuse + per-ad primary rows).
...
### FB launch / insights / AdSwipe operations (learned 2026-07)

- **VERIFY the live Meta state BEFORE launching — launches are NOT idempotent.** Read `<campaign_id>/adsets` → `<adset_id>/ads` first (ad names embed the source row #, so you can map them back); this session nearly re-launched 34 already-live ads (would've been 102 dupes). Don't trust a stale "launch pending" from an earlier turn.
- **CBO vs ABO decides adset budgets.** If the campaign holds the budget (CBO — `get_fb_graph_read <campaign_id> fields=daily_budget` is set), new adsets must carry **NO** budget (passing one → Meta 400 *"You can only set an ad set budget or a campaign budget"*). If the campaign has no budget (ABO), each new adset **NEEDS** its own budget. `validate_launch` passes structurally but does **not** catch this.
- **Placement-coupling → `use_create_from_source: true`.** If `validate_launch` returns `placementCouplingIssue` (*"Meta cannot clone this ad set… create ad sets from source instead"*), the source adset has grandfathered placements Meta `/copies` can't clone → pass `use_create_from_source:true` to `bulk_launch_ads`. Generated adsets land **PAUSED**. If one ad fails with a transient `"fetch failed"`, re-attach just that ad via `launch_ad` into the now-existing adset — don't re-run the whole bulk.
- **Performance reads = ONE bulk CSV export, never row-by-row (user-locked).** `get_fb_account_performance_export(ad_account_id, level=ad|campaign, time_increment=all_days, since, until, fields=[…,actions])` → normalized rows + CSV. Payload exceeds the token limit → saved to a tool-results file; `jq` it to aggregate spend + leads. For these legal campaigns `offsite_conversion.fb_pixel_lead` == `lead` == `onsite_web_lead` (same conversion counted 3×) — use pixel_lead; CPL = spend/leads.
- **The FB connection can break / rotate mid-session.** Symptoms: default lookup *"No healthy Facebook connection"*, the old `connection_id` *"not found"*, and a freshly re-authed connection can show `status:"healthy"` in `list_connections` yet return *"Invalid OAuth access token"* on every Graph call → the **user must RECONNECT Facebook in AdMachin**. Crucially, **AdMachin DB reads (AdSwipe library, `list_ads`/`creatives`/`ad_copies`) keep working when the FB token is dead** — only FB Graph / insights / launch are blocked.
- **AdSwipe dissection:** `export_swipe_creatives_for_agent(serials=[…])` pulls a whole set of swipe ads by their web-UI serial # in one call — full primary/headline/OCR/transcript + resolved Supabase `media_url` (curl the URL + Read to view the image). This is the tool for "dissect swipe ads #A–#B"; it's AdMachin data so it works even when the FB connection is down.

### FB launch specifics — landing URL, bulk-launch timeouts, b-roll gen fields (user-locked 2026-07-09)

- **`landing_url` must be the FULL macro tracking URL passed directly** — never a bare URL + `utm_template` (the launch tool then auto-generates a simplified UTM set that is NOT the media-buyer template). Passed whole, it's stored verbatim and FB resolves the `{{macros}}` at click. Canonical justicecovered template (Depo/JDC) lives in memory `feedback_admachin_launch_landing_url`.
- **`am_mb` is HARDCODED to the operator's username** (Harry = `am_mb=hj`), NOT `am_mb={{media_buyer_code}}` — the macro form stores literally and never resolves.
- **`bulk_launch_ads` can time out ("Launch worker timed out") yet COMPLETE server-side — possibly PARTIALLY** (a Depo run created both adsets + 6 of 8 ads before timing out). NEVER blindly retry — that duplicates adsets. Recover: `list_fb_adsets` + `list_launched_ads(since=)` to see what landed, then fill only the missing ads with single `launch_ad` calls into the EXISTING adset.
- **There is no `delete_launched_ad`.** `pause_launched_ad` stops FB delivery; `archive_launched_ad` is AdMachin-cosmetic only (does NOT touch FB, does NOT stop spend). Wrong-URL live ad → pause it + launch a corrected replacement into the same adset + user hard-deletes the paused one in Meta UI. Launches land adsets PAUSED + ads ACTIVE — no spend until an operator activates the adset in Meta (the money gate). Numbered adsets sequence off the highest existing number.
- **B-roll generation-metadata columns** (`image_generation_model/prompt`, `video_generation_model/prompt`) are NOT writable via MCP `upload_broll` (title/tags/category only) — write them via REST **`PATCH /api/v1/brolls/clips/{id}`** (Bearer `ADMACHIN_PAT`) after upload. Format quirk: image model = provider/model (`"poyo/gpt-image-2"` — bare id 422s), video model = BARE id (`"omni-flash-useapi"` — prefixed 422s); values must be registered Creative Studio model ids. The openapi.json is stale — trust the `upload_broll` response fields. Reference rows: Depo docu b-roll #68–78.
...
- **Stale `WIP: … pre-switch` checkpoint commits on side branches** (auto-saves from session branch-switching) look like unpushed work but are usually already superseded. Verify before worrying or deleting: `git diff origin/main <wip-sha> -- $(git show --name-only --format= <wip-sha>)` — empty output = every file the WIP touched is byte-identical in main → safe to `git branch -D`. Note `git cherry`/"ahead 1" still reports the COMMIT as unmerged even when its content fully landed; and a raw `git diff origin/<branch>..<branch>` on a behind-branch shows misleading mass deletions (that's the branch being old, not real deletions). (Verified on `subtitle-emoji-matching`/8507e29, 2026-07-15.)
...
- **The index is SHARED between concurrent sessions — a plain `git commit` in one session sweeps the OTHER session's staged files into its commit** (happened 2026-07: a memory-pass session staged 3 files; the concurrent session's `git commit` absorbed them under an unrelated message, and the first session's own commit came out with 1 file). Rules: (1) commit by explicit pathspec — `git commit -m "..." -- <your paths>` — never a bare `git commit` when another session may be active; (2) minimize the staged window — commit immediately after staging; (3) if files you staged vanish from your commit, verify CONTENT reached HEAD (`git diff HEAD -- <paths>` empty + grep HEAD for your key lines) instead of assuming loss — it usually landed in the other session's commit; (4) never unstage files you didn't stage (`git restore --staged`) — they're the other session's in-flight work; leave them and commit around them via pathspec.
...
- **The positive control must exercise the WHOLE pipeline, not just the regex** (2026-07). A scan built as `FILES=$(git status --short | awk '{print $2}'); grep -E <pattern> $FILES` passed its pattern-only control but silently scanned NOTHING — zsh passed the newline-joined list as one garbled filename, grep errored, and the `|| echo CLEAN` fallback fired → false "CLEAN". Watch for the tool's own warning/stderr in the output. Prefer scanning `git diff --cached` (pipe content, no file-list plumbing) over filename lists, and scan added lines only (`grep -E '^\+.*<pattern>'`).
- **Never gate a scan verdict on a pipeline ending in `head`/`tail`** (2026-07-15). `… | grep -E <pattern> | head -10 && echo FOUND || echo clean` ALWAYS prints FOUND — the pipeline's exit status is `head`'s (0 even on empty input), not grep's — false "POTENTIAL SECRET" alarm on a clean diff. Capture instead: `matches=$(… | grep -E <pattern>)` then branch on `[ -n "$matches" ]`, or use `grep -q` when output isn't needed.
...
- **Blindly retry a timed-out `bulk_launch_ads`** — the launch worker completes (possibly PARTIALLY) server-side despite the timeout; a retry DUPLICATES adsets. Reconcile first (`list_fb_adsets` + `list_launched_ads(since=)`), then fill only the gaps with single `launch_ad` calls into the existing adset. See "FB launch specifics".
