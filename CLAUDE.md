...
| **Grok Imagine** (i2v) | **KIE** | `grok-imagine/image-to-image` via `jobs/createTask` | cheap | Model id `grok-imagine/image-to-video`. `input.image_urls`, `prompt`, `mode:"normal"`, `duration` STRING 6–30, `resolution:"720p"`, `aspect_ratio:"9:16"`. **THE model for SILENT listener clips** — keeps the mouth CLOSED with natural blinks where every Veo tier animates it (verified 2026-07-11, `interview-scene` skill). Also does normal talking i2v with native audio. |
...
| `interview-scene` | "interview video", "documentary/podcast interview", "two-person interview", "interviewer + subject", "someone being interviewed" | Two-person reverse-angle chest-up talking-head interview: same-room/opposite-wall backdrops, exact-face i2i (`input_urls`, minimal prompt), iPhone-video look, podcast 3/4 gaze, facing-each-other via composite-flip + text-free backdrop, no foreground blob, then one-speaker-per-clip stacked stitch. All 10 locked rules from the Depo build. **Includes the mandatory ANCHOR VERIFICATION PASS (`scripts/interview_anchor_verify.py`) run at the image stage before any video.** |
| `stacked-podcast-broll` | "stacked interview", "stacked podcast", "two-person interview ad with b-roll", "documentary/podcast testimonial ad", "more videos in this format" | FULL stacked-interview pipeline (subject top / interviewer bottom + b-roll over VO). Grok for clean talking + genuine silent listeners; the stereo-48k concat rule (mono segment silences the rest); b-roll-over-voice placement (2s face → b-roll under the line); real-text credibility b-roll (verify facts, never fake gov records); upload b-roll to `/brolls/clips` via `upload_broll_clip` (NOT `/creatives`). Concept→video→b-roll→assembly→library, with the do-not-repeat list. Builds on `interview-scene` for the anchor stage. |
...
- **B-ROLL IS A SEPARATE LIBRARY — do NOT upload b-roll clips as `/creatives` (user-flagged mistake, 2026-07).** The B-Roll page (admachin.com/b-roll) is backed by **`/brolls/clips`**, a different table from `/creatives`. Use `admachin_client.upload_broll_clip(path, title=, clip_category=, tags=, ...)` → `POST /brolls/clips/upload` (multipart, one-step upload+register); `list_broll_clips` to REUSE existing clips before generating new ones (post-op/recovery/scan b-roll already lives there); `delete_broll_clip(id)` needs `?confirm=true`. **Don't send `video_generation_model`** — it 500s server-side. If you mis-upload a b-roll to `/creatives`, move it out (PATCH `project_id=null` — there's no hard DELETE for creatives).
...
- **Cut the chin off in a stacked-interview close-up (user-locked 2026-07).** Panes are wide-ish (~720x640), so a tight/low anchor + a blind fixed crop clips the chin. Compose anchors as a MEDIUM close-up (head+shoulders, headroom above hair, space below chin, face ~40–50% of frame — NOT tight), and crop each pane FACE-AWARE via `scripts/face_crop.py` `pane_crop(clip)` (keeps whole face + chin ≥6% margin, eyeline upper third, matched head size across panes; needs `opencv-python-headless<5`). Verify on a rendered stacked frame.
- **Generate ANY interview/stacked-interview video before the two anchors PASS the anchor-verification pass (user-locked 2026-07).** At the IMAGE stage, run `scripts/interview_anchor_verify.py <subject> <interviewer>` and confirm: opposite 3/4 gazes that FACE EACH OTHER (subject screen-right, interviewer screen-left, never both frontal — "both frontal" is the weird-angle tell), looking-room on the correct side, matched eyeline height + framing scale + camera height, 180° axis, same-room/DIFFERENT-walls backgrounds, consistent lighting. i2i defaults to FRONTAL so the first composite usually fails — re-composite (strong 3/4 turn + off-center placement + flip over a text-free backdrop) until it passes. Every clip inherits the anchor angle, so a bad anchor means re-generating the whole clip set. See `interview-scene` / `stacked-podcast-broll` skills.
