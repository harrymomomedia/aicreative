...
- **Emoji entrance presets** rotate (all settle centered-below): `slide_across` (short slide-in from the left, NOT edge-to-edge), `fly_out` (drifts up to just under the text), `slide_up` (small rise from below), `pop` (scale in place). `EMOJI_ENTER_DUR 0.25s` (SNAPPY), smooth ease `1-(1-p)³`.
- **Emoji is BOUND TO ITS TEXT CARD** — disappears the instant the caption advances to the next card (no cross-card linger; Submagic ties the emoji to the text segment). The fix for "disappears too quickly" was the FASTER entrance (0.25s, settled-visible for most of its short card), NOT a longer linger. `EMOJI_MIN_GAP 0.8s` lightly throttles so most keyword cards show one (gaps on non-keyword cards).
