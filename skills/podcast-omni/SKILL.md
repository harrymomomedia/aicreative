---
name: podcast-omni
description: Produce podcast-style talking-head UGC ad videos with Google Flow's omni-flash model (via useapi, i2v/startImage default) — a weathered persona speaking into a podcast mic, jump-cut. Use when the user says "podcast omni", "omni flash podcast", "make the podcast ad", "Flow podcast video", or wants the headphones-and-condenser-mic podcast creator look produced on Google Flow / omni-flash. Captures the LOCKED prompt recipe + every guardrail/throttle/leak lesson learned the hard way (Chowchilla CCWF campaign, 2026-05).
---

# podcast-omni

Generate **podcast-style talking-head ad clips on Google Flow `omni-flash`** (useapi `google-flow` API, **i2v / `startImage`** mode — the DEFAULT; R2V only if a task explicitly asks for it) and jump-cut them into a ~40–55s ad. Persona = a gpt-image close-up (headphones + studio condenser mic). Reverse-engineered + tuned against the IDENTIFIABLE_PERSON guardrail, the traffic throttle, and a real credit leak.
...
```
Vertical close-up, framed IDENTICALLY from the first frame to the last. The framing is
...
- **NO VISUAL OBJECT NOUNS anywhere — not even in camera boilerplate (user-locked 2026-07-15, applies to omni AND Veo i2v).** The model literalizes positively-stated nouns: "locked-off TRIPOD" spawned actual tripods mid-clip (V/X selfie runs); posture lines like "holding the small wireless mic" re-describe anchor props and invite drift. Say the BEHAVIOR ("framing completely locked", "she stays in place exactly as in the reference"), never the OBJECT. **Negative locks are fine** ("no passing cars, no people walk by" — held the street still); positives literalize, negatives don't.
- **omni i2v is SOFT — it RE-PAINTS the start frame, it does not pin pixels (measured 2026-07-15).** Google's log confirms true I2V mode (`abra_i2v`, `VIDEO_GENERATION_MODE_IMAGE_TO_VIDEO`), but frame-0 lands ~4.2 diff vs a 1.4 codec floor and the clip drifts from there — behaviorally close to single-ref R2V (which also opens near the anchor, ~4.5). Expect per-clip morph outliers; QA with a MID-CLIP frame audit vs the anchor (frame-0 always passes), and re-roll the outliers. Pixel-PINNED start frames exist only on Veo i2v.
- Payload: `model=omni-flash`, `aspectRatio=portrait`, `duration` sized to the line (4 if ≤6 words, 6 if ≤10, else 8), `startImage=<persona asset id>` (i2v — the default; `POD_REF` env only if R2V is explicitly required), `captchaRetry=3`.
- **i2v (`startImage`) is the DEFAULT for omni-flash too (verified 2026-07-15: P + S1-S7, 160+ clips, near-zero guardrail blocks, backgrounds frame-perfect).** It pins the persona PNG's exact background; R2V re-imagines it per clip. Use R2V (`referenceImage_1`) ONLY when a task explicitly asks for it.
...
- One persona PNG per video, used as the `startImage` for ALL its clips (same reference = consistent identity + background).
