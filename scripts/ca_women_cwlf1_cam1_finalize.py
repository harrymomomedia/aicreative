...
INTENDED = {
    1: "i was number four hundred and twelve i know that because they told me when i called",
    2: "four hundred and eleven women had already come forward by the time i found this",
    3: "all of them were sexually abused inside california womens prisons all of them may qualify for significant potential compensation",
    4: "i almost wasnt one of them because i didnt know how close the deadline was",
    5: "completely private and confidential no court the check to find out takes less than a minute and costs nothing",
    6: "tap below find out what number youd be",
}
...
def word_trim(n):
    ...
    tx_path = DISSECT / f"ca_women_cwlf1_cam1_clip{n}" / "transcript.json"
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=4) as ex:
        futs = {ex.submit(word_trim, n): n for n in range(1, 7)}
    ...
    def vc_clip(n):
        ...
        audio_in = f"/tmp/cwlf1_clip{n}_in.mp3"
    ...
    normed = {}
    with ThreadPoolExecutor(max_workers=4) as ex:
        futs = {ex.submit(vc_clip, n): n for n in range(1, 7)}
        for fut in as_completed(futs):
            n = futs[fut]
            try:
                normed[n] = fut.result()
