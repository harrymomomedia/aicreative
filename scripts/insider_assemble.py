...
G  = Path("outputs/depo_insider/clips_grok")
...
S  = Path("outputs/depo_insider/seg"); S.mkdir(parents=True, exist_ok=True)
E  = Path("outputs/depo_insider/edits"); E.mkdir(parents=True, exist_ok=True)
T  = Path("outputs/depo_insider/trim"); T.mkdir(parents=True, exist_ok=True)
...
BEATS = [
 ("doc_0","doc","surv_listen_1","You were a nurse for almost twenty years. When they told you it was a brain meningioma, did you already know what that meant?"),
 ("surv_1","surv","doc_listen_1","I knew exactly what it meant. I'd seen the scans. I just never thought I'd be the one on the table."),
 ("doc_2","doc","surv_listen_2","You gave that injection every day. Did you ever connect it to your own diagnosis?"),
 ("surv_3","surv","doc_listen_2","Not at first. Then a lawyer showed me the studies. Women on the Depo shot for over a year were up to five times more likely to get one. I gave that shot for years."),
 ("surv_4","surv","doc_listen_1","To hundreds of women. And nobody ever told us it could do this. Not the reps, not the label. Nobody."),
 ("doc_5","doc","surv_listen_1","And it turns out you're far from the only one."),
 ("surv_6","surv","doc_listen_2","Not even close. Thousands of women now. Same diagnosis, same shot. There's a federal lawsuit."),
 ("doc_7","doc","surv_listen_2","So for a woman watching this, where does she even start?"),
 ("surv_8","surv","doc_listen_1","You answer a few private questions online. Your diagnosis, how long you were on Depo. Takes about a minute."),
 ("surv_9","surv","doc_listen_2","A lawyer reviews it for free. No court, it's confidential. And even if it was years ago, you may still qualify."),
...
def main():
    ...
    st=E/"insider_stacked.mp4"
    subprocess.run(["ffmpeg","-y","-f","concat","-safe","0","-i",str(lst),"-c:v","libx264","-crf","19",
        "-pix_fmt","yuv420p","-c:a","aac","-b:a","192k","-ar","48000","-ac","2",str(st),"-loglevel","error"])
    normalize(st,E/"insider_stacked_norm.mp4"); web(E/"insider_stacked_norm.mp4",E/"insider_stacked_web.mp4")
    csegs=[]
    for idx,(talk,persona,listener,line) in enumerate(BEATS):
        tf,d=trims[talk]
    ...
    normalize(cut,E/"insider_cut_norm.mp4"); web(E/"insider_cut_norm.mp4",E/"insider_cut_web.mp4")
    for f in ["insider_stacked_norm.mp4","insider_cut_norm.mp4"]:
        import re as _re
