...
INTENDED = {
    1:  "california settled a womens prison abuse case last year over a hundred million dollars",
    2:  "the women who got nothing were not disqualified they just didnt know in time",
    3:  "that is the part nobody tells you the state is not going to call you the facility is not going to send you a letter",
    4:  "the attorneys who built these cases did not have a list of names they had women who found out and came forward on their own",
    5:  "if you were incarcerated at chowchilla chino or another california womens facility and something happened to you",
    6:  "a guard a staff member someone in a position of authority you may qualify for significant potential compensation from those same settlements",
    7:  "you dont need records you dont need proof you dont need a previous attorney",
    8:  "you do not go to court it is completely private you only pay if you win",
    9:  "but you have to come forward before the window closes the women who are already moving are moving right now",
    10: "the ones who wait past the deadline get nothing not because they didnt qualify because they didnt know",
    11: "now you know tap below the check is free and takes less than a minute",
}
...
def word_trim(n):
    ...
    tx_path = DISSECT / f"ca_women_f3_cam1_clip{n}" / "transcript.json"
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=4) as ex:
        futs = {ex.submit(word_trim, n): n for n in range(1, 12)}
    ...
    def vc_clip(n):
        ...
        audio_in = f"/tmp/cw_f3_clip{n}_in.mp3"
    ...
    normed = {}
    with ThreadPoolExecutor(max_workers=4) as ex:
        futs = {ex.submit(vc_clip, n): n for n in range(1, 12)}
        for fut in as_completed(futs):
            n = futs[fut]
            try:
                normed[n] = fut.result()
