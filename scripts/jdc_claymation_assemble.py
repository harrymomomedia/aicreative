...
RAW = CLIPS / "explainer_v1_raw.mp4"
CAPTIONED = CLIPS / "explainer_v1_hormozi.mp4"

IN = 0.2  # skip first 0.2s settle of each clip

# (slug, duration) — durations sum to 54.64 = VO length, aligned to beats
SEGMENTS = [
    # beat 1 — Chicago lockup (0.00–8.12); (slug, dur[, in_point])
    ("clay_chicago_juvie_ext",    1.00),         # 0.00 "When I" — Chicago lockup ext (El/skyline)
    ("lockup_01_intake_walk",     0.90),         # 1.00 "was a kid, Illinois" — kid taken in
