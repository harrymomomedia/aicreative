...
BD = f"{BASE}/backdrops"
CARD_DIR = f"{BASE}/cards_v2"
SEG_DIR = f"{BASE}/segs_v2"
OUT = f"{BASE}/f2_form_explainer_v2.mp4"
...
CARDS = [
    ("c0", "bd0_phone_porch", None, "THE FORM", "What it actually asks", "Chowchilla · CIW · California women's prisons"),
...
def style_a_card(slug, backdrop, kicker, headline, sub):
    ...
    for yy in range(H):
        a = 0 if yy < H * 0.42 else int(215 * min(1.0, (yy - H * 0.42) / (H * 0.32)))
